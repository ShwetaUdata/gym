...
import { calculateBasePrice, calculateFinalPrice, getDayOffer } from '@/utils/pricing';
import { 
...
export function AdminDashboard() {
  ...
  // Expected revenue using offers (finalPrice per client)
  const expectedRevenue = clients.reduce((sum, client) => {
    try {
      ...
      const { finalPrice } = calculateFinalPrice(basePrice, dayOffer);
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        {/* <Card variant="glass">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Expected Revenue (with offers)</p>
              <p className="text-2xl font-bold">₹{Math.round(expectedRevenue).toLocaleString()}</p>
            </div>
          </CardContent>
        </Card> */}
        ...
        <PaymentModal
          ...
          offerAmount={(() => {
            const basePrice = calculateBasePrice(
              paymentClient.membershipType,
              paymentClient.membershipPeriod
            );
            const dayOffer = getDayOffer(new Date(paymentClient.createdAt));
            const { finalPrice } = calculateFinalPrice(basePrice, dayOffer);
            return finalPrice;
          })()}
        />
