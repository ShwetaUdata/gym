...
export function ClientRegistrationForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    slot: '' as 'morning' | 'evening' | 'morning_evening' | 'other' | '',
    membershipPeriod: '',
    customMonths: '',
  ...
  const handleTermsAccept = (typedName: string) => {
    // Update form name with typed name if it's different
    if (typedName && typedName !== formData.name) {
      setFormData(prev => ({ ...prev, name: typedName }));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const client = addClient({
        ...
        slot: formData.slot as 'morning' | 'evening' | 'morning_evening' | 'other',
        membershipType,
    ...
    <Card variant="glass" className="max-w-2xl mx-auto animate-slide-up">
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select
                  value={formData.slot}
                  onValueChange={(value) => setFormData({ ...formData, slot: value as 'morning' | 'evening' | 'other' })}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Member Slot" />
                  </SelectTrigger>
                    <SelectContent>
            ...
            <div className="space-y-3">
              <div className="p-4 rounded-lg bg-secondary/30 border border-border">
                <p className="text-sm text-muted-foreground mb-3">
                  Please read and accept our Terms and Conditions before completing your membership registration.
                </p>
                <Button
                  type="button"
                  variant="outline"
              ...
              <div className="flex items-center space-x-3 p-4 bg-secondary/50 rounded-lg">
                ...
                />
                <Label htmlFor="terms-agree" className="flex-1 cursor-pointer">
                  <span className="text-sm font-medium">
                    I agree to the Terms and Conditions
                  </span>
                  {termsAccepted && (
                    <span className="ml-2 text-xs text-green-600 font-semibold">✓ Accepted</span>
                  )}
                </Label>
              </div>
            </div>
          ...
          <Button type="submit" variant="hero" size="xl" className="w-full" disabled={isSubmitting || !termsAccepted}>
            {isSubmitting ? 'Registering...' : 'Complete Registration & Checkout'}
          </Button>
        </form>
        ...
        <TermsAndConditionsModal
          ...
          clientName={formData.name || 'New Member'}
        />
      </CardContent>
    </Card>
  );
