...
cron.schedule(
  ...
  async () => {
    ...
    try {
      ...
      const clients = await db.all(`SELECT * FROM clients WHERE dob LIKE ?`, [
        todayString,
      ]);
      ...
      for (const client of clients) {
        ...
        if (existing && existing.sent === 1) {
          console.log(
            `Birthday email already sent this year to ${client.clientId}`
          );
          continue;
        }
        ...
        if (result && result.ok) {
          ...
          console.log(
            `Birthday email sent to ${client.name} (${client.email})`
          );
        }
      }
    } catch (error) {
...
app.post("/api/clients/register", async (req, res) => {
  try {
    ...
    const lastClient = await db.get(
      "SELECT clientId FROM clients ORDER BY id DESC LIMIT 1"
    );
    ...
    const newClient = await db.get("SELECT * FROM clients WHERE clientId = ?", [
      nextClientId,
    ]);
...
app.get("/api/clients", async (req, res) => {
  try {
    ...
    const parsedClients = clients.map((client) => {
      let membershipType = client.membershipType;

      // ✅ SAFE JSON parse
      if (typeof membershipType === "string") {
        try {
          membershipType = JSON.parse(membershipType);
        } catch {
          membershipType = null;
        }
      }
...
app.get("/api/clients/:clientId", async (req, res) => {
  try {
    ...
    const payments = await db.all(
      "SELECT * FROM payments WHERE clientId = ? ORDER BY paidDate DESC",
      [req.params.clientId]
    );
    ...
    res.json({
      ...client,
      membershipType:
        typeof client.membershipType === "string"
          ? JSON.parse(client.membershipType)
...
app.put("/api/clients/:clientId", async (req, res) => {
  try {
    ...
    const membershipType = updates.membershipType
      ? JSON.stringify(updates.membershipType)
      : client.membershipType;
...
app.post("/api/payments", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Payment recording failed", details: error.message });
  }
});
...
app.get("/api/payments/:clientId", async (req, res) => {
  try {
    const payments = await db.all("SELECT * FROM payments WHERE clientId = ?", [
      req.params.clientId,
    ]);
    res.json(payments);
  } catch (error) {
...
app.get("/api/emails/:clientId", async (req, res) => {
  try {
    const emails = await db.all(
      "SELECT * FROM emails_sent WHERE clientId = ? ORDER BY sentAt DESC",
      [req.params.clientId]
    );
    res.json(emails);
  } catch (error) {
    res
      .status(500)
      .json({ error: "Failed to fetch email history", details: error.message });
  }
});
