          ...
          {scannedClient && (
            <div className="space-y-4 animate-fade-in">
              ...
              <div className="p-4 rounded-xl bg-secondary/30">
                ...
                {/* Show offer based on type - only one, not both */} 
                {scannedClient.offerApplied && (
                  <div className="mt-3 pt-3 border-t border-border">
                    {scannedClient.offerType === 'freeMonths' ? (
                      <p className="text-sm text-success font-medium">
                        ✓ Offer: {scannedClient.offerValue} month{scannedClient.offerValue && scannedClient.offerValue > 1 ? 's' : ''} free
                      </p>
                    ) : scannedClient.offerType === 'percentage' ? (
                      <p className="text-sm text-success font-medium">
                        ✓ Offer: {scannedClient.offerValue}% discount
                      </p>
                    ) : null}
                  </div>
                )}
