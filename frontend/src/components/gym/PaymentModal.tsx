...
export function PaymentModal({ client, onClose }: PaymentModalProps) {
  ...
  const totalAmount =
    client.finalAmount ||
    calculateDiscountedPrice(client.membershipType, client.membershipPeriod);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const originalAmount = calculateDiscountedPrice(
      ...
      let offerDiscount = 0;
      if (client.offerType === 'percentage' && client.offerValue) {
        offerDiscount = client.offerValue;
      }
      ...
      const paymentData = {
        ...
        remainingAmount: totalAmount - (totalPaid + paymentAmount),
      ...
      toast({
        ...
        description: `${formatCurrency(paymentAmount)} payment recorded for ${
          client.name
        }`,
      });
