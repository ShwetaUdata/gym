...
const birthdayKey = (clientId: string) => {
  const year = new Date().getFullYear();
  return `birthday_sent_${year}_${clientId}`;
};

const isBirthdaySent = (clientId: string) =>
  localStorage.getItem(birthdayKey(clientId)) === "1";

const markBirthdaySent = (clientId: string) =>
  localStorage.setItem(birthdayKey(clientId), "1");

export function BirthdayAlertModal({
  clients,
  onClose,
}: BirthdayAlertModalProps) {
  const { toast } = useToast();
  const [sendingTo, setSendingTo] = useState<string | null>(null);
  const [sentEmails, setSentEmails] = useState<Set<string>>(
    new Set(
      clients.filter((c) => isBirthdaySent(c.clientId)).map((c) => c.clientId)
    )
  );
  ...
  const birthdayClients = clients.filter((client) => {
    ...
    return (
      dobDate.getMonth() + 1 === todayMonth && dobDate.getDate() === todayDay
    );
  });
  ...
  const handleSendBirthdayEmail = async (client: Client) => {
    ...
    try {
      ...
      markBirthdaySent(client.clientId);
          ...
          {birthdayClients.length === 0 ? (
            ...
            <>
              <p className="text-sm text-muted-foreground">
                {birthdayClients.length} member
                {birthdayClients.length > 1 ? "s have" : " has"} birthday today!
              </p>
