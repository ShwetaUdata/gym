...
import { PendingPaymentsModal } from './PendingPaymentsModel';
import { FestivalEmailModal } from './FestivalEmailModel';
  ...
  const handleExportPDF = (options: ExportOptions) => {
    ...
    if (options.sections.clients || options.sections.all) {
      ...
      autoTable(doc, {
        startY: yPos,
        head: [[
          'Client ID', 'Name', 'Mobile', 'Email', 'Offer Applied', 'Occupation',
    ...
    if (options.sections.payments || options.sections.all) {
      ...
      if (allPayments.length > 0) {
        ...
        autoTable(doc, {
          startY: yPos,
          head: [['Client ID', 'Name', 'FinalAmount', 'PaidAmount', 'PaidDate', 'Notes']],
    ...
    // Emails Section
    // if (options.sections.emails || options.sections.all) {
    //   doc.text('Email Records - Not Available', 14, yPos);
    //   yPos += 10;
    // }
